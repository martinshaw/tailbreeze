export const minHeightStyle = (() => {
    ...
    type Keys = keyof typeof minHeightAmounts;
    type Values = typeof minHeightAmounts[Keys];

    return Object.fromEntries(
        Object.entries(minHeightAmounts)
            .map(
                ([v, k]) => {
                    const style = minHeightAmounts[v as Keys];
                    return [v, { minHeight: style }];
                }
            )
    ) as {[key in Keys]: any};
})();
