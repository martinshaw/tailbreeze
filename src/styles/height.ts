export const heightStyle = (() => {
    ...
    type Keys = keyof typeof heightAmounts;
    type Values = typeof heightAmounts[Keys];

    return Object.fromEntries(
        Object.entries(heightAmounts)
            .map(
                ([v, k]) => {
                    const style = heightAmounts[v as Keys];
                    return [v, { height: style }];
                }
            )
    ) as {[key in Keys]: any};
})();
